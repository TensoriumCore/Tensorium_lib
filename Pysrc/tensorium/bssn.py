...
BSSNGrid = _C.bssn.BSSNGrid
GaugeParameters = _C.bssn.GaugeParameters
BSSNRKStepper = _C.bssn.BSSNRKStepper
...
def has_twopunctures_c() -> bool:
    ...
    return bool(_C.bssn.has_twopunctures_c())
...
def apply_radiative_halos(grid: BSSNGrid) -> None:
    """Apply the BoundaryRadiative halo operator to all grid fields."""
    _C.bssn.apply_radiative_halos(grid)


def project_state(grid: BSSNGrid) -> None:
    """Project BSSN algebraic constraints on the current state."""
    _C.bssn.project_state(grid)


def zero_z4c_fields(grid: BSSNGrid) -> None:
    """Set Theta/Z fields to zero."""
    _C.bssn.zero_z4c_fields(grid)


def set_spatial_derivative_order(order: int) -> None:
    """Set spatial finite-difference order (4 or 6)."""
    _C.bssn.set_spatial_derivative_order(int(order))


def spatial_derivative_order() -> int:
    """Current spatial finite-difference order."""
    return int(_C.bssn.spatial_derivative_order())


def set_fd_dx(dx: float) -> None:
    """Set FD spacing used by derivative helpers."""
    _C.bssn.set_fd_dx(float(dx))


def compute_dt_cfl(grid: BSSNGrid, cfl: float, gauge_speed: float = 1.0, padding: int = 4) -> float:
    """Compute stable timestep estimate from CFL control."""
    return float(_C.bssn.compute_dt_cfl(grid, float(cfl), float(gauge_speed), int(padding)))
...
def minkowski(grid: BSSNGrid, M: float = 1.0, xc: float = 0.0, yc: float = 0.0, zc: float = 0.0,
              r_floor: float = 1e-6) -> None:
    """Initialize Minkowski data on the BSSN grid."""
    _C.bssn.minkowski(grid, M, xc, yc, zc, r_floor)


def schwarzschild_isotropic(grid: BSSNGrid, M: float, xc: float = 0.0, yc: float = 0.0,
                            zc: float = 0.0, r_floor: float = 1e-6) -> None:
    """Initialize isotropic Schwarzschild puncture data."""
    _C.bssn.schwarzschild_isotropic(grid, M, xc, yc, zc, r_floor)


def kerr_schild_single(grid: BSSNGrid, M: float, a: float, xc: float = 0.0, yc: float = 0.0,
                       zc: float = 0.0, r_floor: float = 1e-6) -> None:
    """Initialize a single Kerr-Schild black hole."""
    _C.bssn.kerr_schild_single(grid, M, a, xc, yc, zc, r_floor)


def binary_bowen_york_puncture_init(grid: BSSNGrid, m1: float, x1: float, y1: float, z1: float,
                                    P1, S1, m2: float, x2: float, y2: float, z2: float, P2, S2,
                                    r_floor: float = 1e-6) -> None:
    """Initialize binary Bowen-York puncture data."""
    _C.bssn.binary_bowen_york_puncture_init(grid, m1, x1, y1, z1, P1, S1, m2, x2, y2, z2, P2, S2,
                                            r_floor)


def binary_bowen_york_puncture_interpolated_init(
    grid: BSSNGrid, m1: float, x1: float, y1: float, z1: float, P1, S1, m2: float, x2: float,
    y2: float, z2: float, P2, S2, interp_seed_n: int = 64, r_floor: float = 1e-6
) -> None:
    """Initialize interpolated binary Bowen-York puncture data."""
    _C.bssn.binary_bowen_york_puncture_interpolated_init(
...
def binary_bowen_york_puncture_twopunctures_c_init(
    ...
    with _temporary_env(env_updates):
        _C.bssn.binary_bowen_york_puncture_twopunctures_c_init(
            grid, m1, x1, y1, z1, P1, S1, m2, x2, y2, z2, P2, S2, interp_seed_n, r_floor
        )
...
def run_moving_puncture_interpolate_from_env() -> tuple[BSSNGrid, float]:
    """
    ...
    grid = BSSNGrid(n, n, n, ng, spacing, spacing, spacing)
    grid.set_origin(
