...
tensorium::Vector<double> numpy_to_vector(py::array_t<double> arr) {
    ...
    return tensorium::Vector<double>(
        std::vector<double>((double *)buf.ptr, (double *)buf.ptr + buf.shape[0]));
}

tensorium::Tensor<double, 2> numpy_to_tensor2(py::array_t<double> arr) {
    auto buf = arr.request();
    if (buf.ndim != 2)
    ...
    tensorium::Tensor<double, 2> tensor(dims);
...
PYBIND11_MODULE(tensorium, m) {
    ...
    py::class_<tensorium::Tensor<double, 4>>(tns, "Tensor4d")
        ...
        .def("__getitem__",
             [](const tensorium::Tensor<double, 4>        &T,
        ...
        .def("__setitem__", [](tensorium::Tensor<double, 4>              &T,
                               std::tuple<size_t, size_t, size_t, size_t> idx, double v) {
    ...
    py::class_<tensorium::Tensor<double, 2>>(tns, "Tensor2d")
        ...
        .def(
            "contract",
            [](const tensorium::Tensor<double, 2> &T, size_t i, size_t j) {
                if (i >= 2 || j >= 2 || i == j)
        ...
        .def(
            "get",
            [](const tensorium::Tensor<double, 2> &T, size_t i, size_t j) { return T(i, j); },
        ...
        .def("__getitem__", [](const tensorium::Tensor<double, 2> &T,
                               std::pair<size_t, size_t> idx) { return T(idx.first, idx.second); })

        .def("__setitem__", [](tensorium::Tensor<double, 2> &T, std::pair<size_t, size_t> idx,
                               double val) { T(idx.first, idx.second) = val; });
    py::class_<tensorium_RG::RiemannTensor<double>>(tns, "Riemann")
        .def_static("print_componentwise",
                    &tensorium_RG::RiemannTensor<double>::print_componentwise, py::arg("R"),
    ...
    tns.def("compute_christoffel",
            [](const Vector<double> &X, const tensorium::Tensor<double, 2> &g,
               const tensorium::Tensor<double, 2> &ginv, const std::string &metric_type, double M,
               double a) {
                auto metric = tensorium_RG::Metric<double>(metric_type, M, a);
                return tensorium::compute_christoffel(X, 1e-5, g, ginv, metric);
            });
    tns.def("compute_riemann_tensor",
            [](const Vector<double> &X, const std::string &metric_type, double M, double a) {
                auto metric = tensorium_RG::Metric<double>(metric_type, M, a);
                return tensorium::compute_riemann_tensor<double>(X, 1e-5, metric);
            });
    tns.def(
        "contract_riemann_to_ricci",
        [](const tensorium::Tensor<double, 4> &R, const tensorium::Tensor<double, 2> &ginv) {
            return tensorium::contract_riemann_to_ricci(R, ginv);
        },
        py::arg("riemann"), py::arg("g_inv"),
        "Contract a Riemann tensor R_{ρσμν} to the Ricci tensor R_{μν} using g^{ρσ}");
    tns.def(
        "compute_ricci_scalar",
        [](const tensorium::Tensor<double, 2> &Ricci, const tensorium::Tensor<double, 2> &ginv) {
            return tensorium::compute_ricci_scalar(Ricci, ginv);
        },
        py::arg("ricci"), py::arg("g_inv"),
        "Compute the Ricci scalar R from the Ricci tensor R_{μν} and the inverse metric g^{μν}");

    tns.def("print_ricci_tensor", &tensorium::print_ricci_tensor<double>, py::arg("R"),
            "Print components of the Ricci tensor");

    tns.def("print_ricci_scalar", &tensorium::print_ricci_scalar<double>, py::arg("Ricci_tensor"),
            py::arg("g_inv"), "Print the Ricci scalar from Ricci tensor and inverse metric");

    tns.def("inv_mat_tensor", &tensorium::inv_mat_tensor<double>, "Inverse of a 2D tensor");
    ...
    py::class_<Matrix<float>>(m, "Matrix")
        ...
        .def("__getitem__", [](const Matrix<float>      &m,
                               std::pair<size_t, size_t> idx) { return m(idx.first, idx.second); })
        .def("__setitem__", [](Matrix<float> &m, std::pair<size_t, size_t> idx,
                               float val) { m(idx.first, idx.second) = val; })
        .def("rows", [](const Matrix<float> &m) { return m.rows; })
        .def("cols", [](const Matrix<float> &m) { return m.cols; })
        .def("fill",
             [](Matrix<float> &m, const std::vector<std::vector<float>> &values) {
    ...
    // Math backend
    tns.def("add_vec", &add_vec<float>, "Add two vectors");
    tns.def("sub_vec", &sub_vec<float>, "Subtract two vectors");
    tns.def("scl_vec", &scl_vec<float>, "Scale a vector by scalar");
    tns.def("dot_vec", &dot_vec<float>, "Dot product between two vectors");
    tns.def("norm_1", &norm1_vec<float>, "L1 norm of vector");
    tns.def("norm_2", &norm2_vec<float>, "L2 norm of vector");
    tns.def("norm_inf", &normInf_vec<float>, "Infinity norm of vector");
    tns.def("cosine", &cosine_vec<float>, "Cosine similarity between vectors");
    tns.def("lerp", &lerp_vec<float>, "Linear interpolation between two vectors");
    tns.def("linear_comb", &tensorium::linear_combination_vec<float>,
            "Linear combination of vectors");
    tns.def("cross", &cross_vec<float>, "Cross product (only defined for 3D vectors)");

    tns.def(
        "tensor_product",
        [](const tensorium::Tensor<double, 2> &A, const tensorium::Tensor<double, 2> &B) {
            return mul_tensor(A, B);
        },
        py::arg("A"), py::arg("B"), "Tensor (outer) product of two tensors");
    tns.def("contract_tensor", [](const tensorium::Tensor<double, 4> &T, size_t i, size_t j) {
        if (i >= 4 || j >= 4 || i == j)
            throw std::invalid_argument("Invalid contraction indices");
    ...
    tns.def("mul_vec", &mul_vec<float>, "Multiply matrix by vector");
    tns.def("inverse_mat", &inverse_mat<float>, "Inverse of a matrix");
    tns.def("det_mat", &det_mat<float>, "Determinant of a matrix");
    tns.def("rank_mat", &rank_mat<float>, "Rank of a matrix");

    tns.def("gauss_solve", &gauss_solve<float>, "Solve linear system with Gauss elimination");
    tns.def("jacobi_solve", &jacobi_solve<float>, py::arg("A"), py::arg("b"),
            py::arg("tol") = 1e-6f, py::arg("max_iter") = 1000, "Jacobi iterative method");
    tns.def("row_echelon", &tensorium::row_echelon<float>, py::arg("A"), py::arg("b") = nullptr,
            py::arg("eps") = 1e-12f, "Convert matrix A (and optionally b) to row echelon form");
}
