...
void export_slice(const tensorium_RG::BSSNGridSoA<double> &grid,
...
int main() {
    ...
    run_one(
        "Schwarzschild",
        [](tensorium_RG::BSSNGridSoA<double> &g) {
    ...
    tensorium_RG::BSSNGridSoA<double> g(cfg.nx, cfg.nx, cfg.nx, cfg.ng, cfg.dx, cfg.dx, cfg.dx);
    const double                      half = 0.5 * cfg.nx * cfg.dx;
    g.x0 = -half;
    g.y0 = -half;
    g.z0 = -half;
    ...
    rhs.st = g.chi.st;
    rhs.data = tensorium_RG::aligned_alloc_n<double>(rhs.st.nx_tot * rhs.st.ny_tot * rhs.st.nz_tot);
    tensorium_RG::bssn::compute_rhs_chi(g, rhs);
    double max_rhs = 0.0;
    size_t I0, I1, J0, J1, K0, K1;
