...
size_t find_index_near_x(const tensorium_RG::BSSNGridSoA<double> &grid, double x_phys) {
...
SlicePlaneSpec realize_request(const tensorium_RG::BSSNGridSoA<double> &grid,
...
void export_scalar_slice(const tensorium_RG::BSSNGridSoA<double> &grid,
...
void export_vector_slices(const tensorium_RG::BSSNGridSoA<double> &grid,
...
void add_debug_perturbation(tensorium_RG::BSSNGridSoA<double> &grid, double eps) {
    size_t I0, I1, J0, J1, K0, K1;
    grid.domain_bounds(I0, I1, J0, J1, K0, K1);
...
void apply_gauge_update(tensorium_RG::BSSNGridSoA<double> &grid, const tensorium_RG::Field3D<double> &rhs_alpha,
...
void export_all_slices(const tensorium_RG::BSSNGridSoA<double> &grid,
                       const tensorium_RG::Field3D<double> &rhs_alpha,
                       const tensorium_RG::Field3D<double> rhs_beta[3],
                       const tensorium_RG::Field3D<double> rhs_B[3],
...
void run_case(const char *label, InitFn init, const DemoConfig &cfg, CaseOptions case_opts,
    ...
    tensorium_RG::bssn::compute_rhs_Gamma(grid, rhs_Gamma, cfg.padding);
    tensorium_RG::bssn::compute_rhs_alpha(grid, rhs_alpha, cfg.padding);
    tensorium_RG::bssn::compute_rhs_beta(grid, rhs_beta, params, cfg.padding);
    tensorium_RG::bssn::compute_rhs_B(grid, rhs_Gamma, rhs_B, params, cfg.padding);
    ...
    if (cfg.apply_update && case_opts.apply_update) {
        ...
        const auto monitor = tensorium_RG::bssn::project_and_monitor(grid, grid.Hc, grid.Mc,
                                                                     ...
                                                                     0.0, 0.0, cfg.padding);
        tensorium_RG::bssn::print_constraint_monitor(
