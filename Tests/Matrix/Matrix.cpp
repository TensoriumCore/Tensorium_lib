...
#pragma omp for schedule(static)
        ...
        double elapsed_blas = std::chrono::duration<double>(t3 - t2).count();
        double gflops_blas = static_cast<double>(flops / 1e9L) / elapsed_blas;
        ...
        double speedup = gflops_our / gflops_blas;
        std::cout << "\n[Summary]\n";
...
int matrix_tests() {
    ...
    gamma.print();
    auto R = tensorium::compute_riemann_tensor<double>(
        X, 1e-5, tensorium_RG::Metric<double>("kerr_schild", 1.0, 0.935));
