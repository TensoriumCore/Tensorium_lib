...
def main():
    default = Path("rhs_Gamma_slice_schwarzschild.csv")
    path = Path(sys.argv[1]) if len(sys.argv) > 1 else default
    if not path.exists():
        print(f"Slice '{path}' not found. Run TensoriumGammaEvolutionDemo to generate the CSV.",
              file=sys.stderr)
        sys.exit(1)
    ...
    ax.set_xlabel("y")
    ax.set_ylabel("x")
    ax.set_title(f"Γ RHS slice: {path.name}")
