...
rebuild_moving_puncture_binary_hierarchy(
    ...
    const tensorium_RG::z4c::MovingPunctureEnvConfig &cfg,
    const tensorium_RG::z4c::ProjectionConfig &proj_cfg,
    const tensorium::backend::Options &backend_options, const std::array<double, 3> &p1,
    const std::array<double, 3> &p2) {
    Grid root_copy = make_grid_like(previous->root_grid());
    tensorium_RG::z4c::fmr::detail::copy_evolved_state(previous->root_grid(), root_copy);
    ...
    auto hierarchy = std::make_unique<BinaryMovingPunctureHierarchy>(
        root_copy, hierarchy_cfg, backend_options, cfg.padding);
...
resolve_moving_puncture_backend(const tensorium_RG::z4c::MovingPunctureEnvConfig &cfg) {
    auto backend = cfg.backend_options;
    if (cfg.backend_auto) {
        backend.backend = tensorium::cuda::is_available() ? tensorium::backend::Kind::CUDA
...
int main(int argc, char **argv) {
    ...
    std::cout << "[mesh] nx=" << cfg.nx << " ny=" << cfg.ny << " nz=" << cfg.nz
              ...
              << cfg.nz * cfg.spacing << ")\n";
    std::cout << "[backend] selected=" << tensorium::backend::to_string(backend_options.backend);
    ...
    if (use_fmr) {
        if (use_bbh_split) {
            binary_hierarchy = std::make_unique<BinaryMovingPunctureHierarchy>(
                root_grid, binary_hierarchy_cfg, backend_options, cfg.padding);
            ...
            hierarchy =
                std::make_unique<MovingPunctureHierarchy>(root_grid, level_cfgs, backend_options,
                                                          cfg.padding);
                    ...
                    if (need_full_rebuild) {
                        ...
                        try {
                            auto next_hierarchy = rebuild_moving_puncture_binary_hierarchy(
                                binary_hierarchy.get(), cfg, proj_cfg, backend_options,
                                puncture_tracker.p1, puncture_tracker.p2);
