...
double interior_max_rhs(const tensorium_RG::Z4cGridSoA<double> &grid,
...
void export_rhs_slice(const tensorium_RG::Z4cGridSoA<double> &grid,
...
double metric_inverse_residual(const tensorium_RG::Z4cGridSoA<double> &grid, size_t padding) {
    size_t I0, I1, J0, J1, K0, K1;
    grid.domain_bounds(I0, I1, J0, J1, K0, K1);
    const size_t i_begin = I0 + padding;
    const size_t i_end = I1 - padding;
...
void run_case(const char *label, InitFn init, const DemoConfig &cfg, const std::string &csv_path,
    ...
    tensorium_RG::z4c::compute_rhs_A_tilde(grid, rhs, padding);
