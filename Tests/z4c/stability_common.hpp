...
inline RunSummary summarize_grid(const Grid &grid, size_t padding) {
    ...
    tensorium_RG::z4c::compute_z4c_constraints(mutable_grid, grid.Ricci, scratch.H, scratch.M,
                                                 scratch.C, 0.0, std::numeric_limits<double>::max(),
                                                 0.0, 0.0, 0.0, false);
    const auto stats = tensorium_RG::z4c::compute_constraint_monitor(grid, scratch.H, padding);
...
RunSummary run_stability_case_impl(const std::string &label, const StabilityRunConfig &cfg,
    ...
    if (cfg.simulate_only) {
        for (size_t step = 0; step < cfg.steps; ++step) {
            const double dt = tensorium_RG::z4c::compute_dt_cfl(grid, control, cfg.padding);
            ...
            const auto stats = tensorium_RG::z4c::compute_constraint_monitor(grid, constraint_fields.H,
                                                                              cfg.padding);
            if (logger.enabled()) {
    ...
    tensorium_RG::z4c::GaugeParameters<double> gauge_params;
    ...
    tensorium_RG::z4c::compute_z4c_constraints(grid, grid.Ricci, constraint_fields.H,
...
inline RunSummary run_stability_case(const std::string &label, const StabilityRunConfig &cfg,
    ...
    if (cfg.boundary == BoundaryType::Clamp)
        return run_stability_case_impl<tensorium_RG::z4c::BoundaryClamp>(label, cfg, initializer,
                                                                          control, allow_failure);
    return run_stability_case_impl<tensorium_RG::z4c::BoundarySponge>(label, cfg, initializer,
                                                                       control, allow_failure);
}

inline tensorium_RG::z4c::CFLControl<double> make_cfl_control(const StabilityRunConfig &cfg) {
    tensorium_RG::z4c::CFLControl<double> ctrl;
    ctrl.cfl = cfg.cfl;
