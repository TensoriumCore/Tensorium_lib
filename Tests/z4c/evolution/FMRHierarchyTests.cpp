...
inline tensorium_RG::z4c::fmr::PatchBox fine_patch() {
    return tensorium_RG::z4c::fmr::PatchBox{6, 14, 6, 14, 6, 14};
}
...
REGISTER_TEST("z4c.fmr.prolongation_is_exact_for_linear_fields",
    ...
    tensorium_RG::z4c::apply_halos_grid<tensorium_RG::z4c::BoundaryRadiative>(root);
...
REGISTER_TEST("z4c.fmr.fine_boundary_evolves_physical_shell",
              ...
              []() {
                  ...
                  tensorium_RG::z4c::Z4cRKStepper<double, FineBoundary> stepper(fine, 4);
                  ...
                  const tensorium_RG::z4c::fmr::ScopedParentInterpolationContext<double> scope(&ctx);
...
REGISTER_TEST("z4c.fmr.parent_interpolation_boundary_blends_in_time",
              "Fine halo fills use a temporal blend of the parent states", []() {
                  ...
                  const tensorium_RG::z4c::fmr::ScopedParentInterpolationContext<double> scope(&ctx);
                  tensorium_RG::z4c::apply_halos_grid<FineBoundary>(fine);
...
REGISTER_TEST("z4c.fmr.parent_interpolation_boundary_tracks_stage_fraction",
...
REGISTER_TEST("z4c.fmr.initial_shell_blending_matches_parent_at_interface",
...
REGISTER_TEST("z4c.fmr.centered_core_blending_keeps_parent_outer_collar",
              ...
              []() {
                  ...
                  const tensorium_RG::z4c::fmr::PatchBox centered_patch{8, 16, 8, 16, 8, 16};
...
REGISTER_TEST("z4c.fmr.hierarchy_step_keeps_levels_finite",
...
REGISTER_TEST("z4c.fmr.moving_puncture_auto_boxes_cover_the_binary",
              ...
              []() {
                  ...
                  const auto levels = tensorium_RG::z4c::build_moving_puncture_fmr_levels(root, cfg);
...
REGISTER_TEST("z4c.fmr.moving_puncture_auto_boxes_do_not_outgrow_parents",
              "Automatic moving-puncture FMR boxes stay no larger than their parent grids",
              []() {
                  tensorium_RG::z4c::MovingPunctureEnvConfig cfg;
                  cfg.nx = 224;
                  cfg.ny = 224;
                  cfg.nz = 224;
                  cfg.spacing = 36.0 / 224.0;
                  ...
                  TENSORIUM_TEST_ASSERT(level1.dims.nx <= root.dims.nx);
                  TENSORIUM_TEST_ASSERT(level1.dims.ny <= root.dims.ny);
                  TENSORIUM_TEST_ASSERT(level1.dims.nz <= root.dims.nz);
                  TENSORIUM_TEST_ASSERT(level2.dims.nx <= level1.dims.nx);
                  TENSORIUM_TEST_ASSERT(level2.dims.ny <= level1.dims.ny);
                  TENSORIUM_TEST_ASSERT(level2.dims.nz <= level1.dims.nz);

                  const double finest_half_width =
                      0.5 * std::min({double(level2.dims.nx) * level2.dx,
                                      double(level2.dims.ny) * level2.dy,
                                      double(level2.dims.nz) * level2.dz});
                  TENSORIUM_TEST_ASSERT(finest_half_width > cfg.separation);
              });

REGISTER_TEST("z4c.fmr.moving_puncture_outer_box_half_width_controls_nested_levels",
...
REGISTER_TEST("z4c.fmr.moving_puncture_centered_levels_follow_requested_center",
              ...
              []() {
                  ...
                  const auto levels =
                      tensorium_RG::z4c::build_moving_puncture_fmr_levels(root, cfg, center);
                  TENSORIUM_TEST_ASSERT(levels.size() == 2);
...
REGISTER_TEST("z4c.fmr.twopunctures_repeated_init_reuses_cached_solve",
              ...
              []() {
                  ...
                  tensorium_RG::z4c::center_cell_centered_origin(coarse);
                  tensorium_RG::z4c::center_cell_centered_origin(fine);
