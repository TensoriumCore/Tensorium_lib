...
tensorium_RG::z4c::GaugeParameters<double> make_schwarzschild_gauge() {
...
void log_progress(size_t step, const GaugeSnapshot &snapshot,
                  const tensorium_RG::z4c::ConstraintMonitorStats &stats) {
...
REGISTER_TEST(
    "z4c.evolution.ccz4_schwarzschild_stability",
    "Ensure CCZ4/Z4c evolution keeps Schwarzschild data stationary", []() {
        ...
        auto gather_constraints = [&]() {
            ...
            auto stats = tensorium_RG::z4c::compute_constraint_monitor(grid, constraint_fields.H,
                                                                        padding);
            tensorium_RG::z4c::populate_constraint_norms(grid, constraint_fields.M, stats, padding);
            return stats;
        };
        ...
        for (size_t step = 1; step <= steps; ++step) {
            const double dt = tensorium_RG::z4c::compute_dt_cfl(grid, control, padding);
