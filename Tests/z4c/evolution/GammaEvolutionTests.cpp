...
void alloc_rhs(const tensorium_RG::Z4cGridSoA<double> &grid,
...
REGISTER_TEST("z4c.evolution.gamma_contracted", "Γ^i RHS behaves across datasets", []() {
    ...
    auto run_case = [&](auto init_fn, const char *label, double far_tol) {
        ...
        tensorium_RG::z4c::compute_rhs_Gamma(grid, rhs, grid.Z, grid.Theta, params, padding);
...
REGISTER_TEST("z4c.evolution.gamma_theta_ignore_auxiliary_z",
              "Gamma and Theta RHS derive Z from contracted Gamma, not the stored Z field", []() {
                  constexpr size_t padding = 4;
                  tensorium_RG::Z4cGridSoA<double> grid(24, 22, 20, 4, 0.25, 0.22, 0.20);
                  ...
                  tensorium_RG::z4c::compute_rhs_Gamma(grid, rhs_gamma_a, grid.Z, grid.Theta,
                                                       params, padding);
                  tensorium_RG::z4c::compute_rhs_Theta(grid, rhs_theta_a, params, padding);
                  ...
                  tensorium_RG::z4c::compute_rhs_Gamma(grid, rhs_gamma_b, grid.Z, grid.Theta,
                                                       params, padding);
                  tensorium_RG::z4c::compute_rhs_Theta(grid, rhs_theta_b, params, padding);
...
REGISTER_TEST("z4c.evolution.theta_z4_trace_cache_matches_direct_contraction",
                  ...
                  for (const int order : {4, 6}) {
                      ...
                      for (size_t i = I0 + padding; i < I1 - padding; ++i)
                          for (size_t j = J0 + padding; j < J1 - padding; ++j)
                              for (size_t k = K0 + padding; k < K1 - padding; ++k) {
                                  double RicciZ4[6];
                                  tensorium_RG::z4c::compute_RicciZ4_core(
                      ...
                      tensorium_RG::z4c::compute_rhs_Theta(grid, rhs_direct, params, padding);
                      tensorium_RG::z4c::compute_rhs_Theta(grid, rhs_cached, params, padding,
                                                            &z4_trace_cache);
...
REGISTER_TEST("z4c.evolution.z_rhs_is_diagnostic_only",
              "Z_i RHS stays zero even when evolve_Z is enabled", []() {
                  ...
                  tensorium_RG::z4c::compute_rhs_Z(grid, rhs_Z, params, padding);
