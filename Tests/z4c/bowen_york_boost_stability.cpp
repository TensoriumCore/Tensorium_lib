...
REGISTER_TEST(
    "z4c.viz.moving_puncture", "Export CSV slices of a moving spinning black hole", []() {
        const auto mp_env = tensorium_RG::z4c::load_moving_puncture_env();
        ...
        std::cout << "[num] spatial_derivative_order=" << mp_env.spatial_derivative_order
                  << std::endl;
        tensorium_RG::z4c::apply_boundary_configuration(mp_env);
        ...
        const char *interp_mode = tensorium_RG::z4c::moving_puncture_interpolated_mode_name();
        ...
        tensorium_RG::z4c::GaugeParameters<double> params = mp_env.gauge_params;
        ...
        const auto bc_char = tensorium_RG::z4c::configure_boundary_characteristics_from_state(
            grid, mp_env, params);
        ...
        auto collect_diagnostics =
            ...
            tensorium_RG::z4c::compute_z4c_constraints(grid, grid.Ricci, constraint_scratch.H,
                                                         constraint_scratch.M, constraint_scratch.C,
                                                         r_min, r_max, 0.0, 0.0, 0.0, false);
            auto stats = tensorium_RG::z4c::compute_constraint_monitor(grid, constraint_scratch.H,
                                                                        cfg.padding);
            tensorium_RG::z4c::populate_constraint_norms(grid, constraint_scratch.M, stats,
                                                          cfg.padding);
            return stats;
        };
        ...
        auto log_diagnostics = [&](size_t step_index,
                                   const tensorium_RG::z4c::ConstraintMonitorStats *precomputed =
                                       nullptr) {
        ...
        for (size_t n = 0; n < cfg.steps; ++n) {
            const double dt = tensorium_RG::z4c::compute_dt_cfl(
            ...
            tensorium_RG::z4c::ConstraintMonitorStats stats;
...
REGISTER_TEST("z4c.viz.moving_puncture_interpolate",
              "Export CSV slices of moving punctures using interpolated initial data", []() {
                  ...
                  const auto  it = std::find_if(
                      tests.begin(), tests.end(), [](const tensorium::tests::TestCase &tc) {
                          return tc.name == "z4c.viz.moving_puncture";
                      });
                  if (it == tests.end()) {
...
REGISTER_TEST(
    "z4c.init.kerr_schild", "Print formatted BSSN fields after Kerr–Schild initialization", []() {
        tensorium::tests::StabilityRunConfig cfg;
        cfg.nx = 96;
        cfg.ny = 96;
        cfg.nz = 96;
