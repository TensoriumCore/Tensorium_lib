...
using namespace tensorium_RG::z4c;
...
PunctureCandidate find_min_candidate_in_window(const Z4cGridSoA<double>& grid, size_t k_local,
...
void refine_candidate_xy(const Z4cGridSoA<double>& grid, PunctureCandidate& cand, size_t k_local) {
    if (!cand.found) {
        return;
    }
...
PuncturePlaneSample sample_local_equatorial_minima(const Z4cGridSoA<double>& grid,
...
void export_xy_midplane_slice_csv(const Z4cGridSoA<double>& grid, const MPIDomain& domain,
...
RunConfig make_default_config() {
    RunConfig cfg;
    const auto mp = tensorium_RG::z4c::load_moving_puncture_env();
...
template <typename T>
void initialize_binary_puncture(Z4cGridSoA<T>& grid, const RunConfig& cfg) {
...
int main(int argc, char** argv) {
    ...
    tensorium_RG::z4c::apply_boundary_faces(cfg.boundary_faces);
    tensorium_RG::z4c::BoundaryRadiative::set_rhs_collar_width(cfg.radiative_collar_width);
    tensorium_RG::z4c::BoundaryRadiative::set_sponge(cfg.sponge.enabled, cfg.sponge.width,
                                                      cfg.sponge.strength, cfg.sponge.exponent);
    tensorium_RG::z4c::configure_ko_boundary_taper(cfg.ko_boundary_width,
                                                    cfg.ko_boundary_floor);
    ...
    if (cfg.slice_export_stride > 0) {
        stepper.set_snapshot_callback(
            [&](const Z4cGridSoA<double>& local_grid, size_t step_index) {
    ...
    stepper.set_constraint_callback(
        [&](const Z4cGridSoA<double>&, const GlobalConstraintStats& stats) {
            last_stats = stats;
