...
struct Z4cInvariantStats : public tensorium_RG::z4c::InvariantStats {
...
inline Z4cInvariantStats compute_invariants(const tensorium_RG::Z4cGridSoA<double> &grid,
                                            size_t padding, double r_min = 0.0,
                                            double r_max = std::numeric_limits<double>::max(),
                                            double xc = 0.0, double yc = 0.0, double zc = 0.0) {
    Z4cInvariantStats stats;

    const auto inv_stats = tensorium_RG::z4c::compute_invariant_stats(grid, padding, r_min, r_max,
                                                                      xc, yc, zc);
    static_cast<tensorium_RG::z4c::InvariantStats &>(stats) = inv_stats;
    ...
    auto &mutable_grid = const_cast<tensorium_RG::Z4cGridSoA<double> &>(grid);
    tensorium_RG::z4c::compute_z4c_constraints(mutable_grid, grid.Ricci, H_tmp, M_tmp, C_tmp,
...
inline double max_gamma_violation_z4c(const tensorium_RG::Z4cGridSoA<double> &grid,
                                      size_t padding, double r_min = 0.0,
                                      double r_max = std::numeric_limits<double>::max(),
                                      double xc = 0.0, double yc = 0.0, double zc = 0.0) {
    size_t I0, I1, J0, J1, K0, K1;
    ...
    for (size_t i = i0; i < i1; ++i)
        for (size_t j = j0; j < j1; ++j)
            for (size_t k = k0; k < k1; ++k) {
                ...
                double div[3];
                tensorium_RG::z4c::metric_inverse_divergence(grid, i, j, k, div);
