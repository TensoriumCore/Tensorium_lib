...
namespace tensorium_RG {
			...
			public:
				...
				__attribute__((always_inline, hot, flatten))
					static inline ChristoffelSym<T> compute_christoffel(
							...
							const std::function<void(const tensorium::Vector<T>&, tensorium::Tensor<T, 2>&)>& metric_generator)
					{
	...
	template <typename T>
		__attribute__((always_inline, hot, flatten))
		inline tensorium::Tensor<T, 2> inv_mat_tensor_local(const tensorium::Tensor<T, 2>& g) {
	...
	template<typename T>
		...
		inline void calculate_christoffel_at_offset(
				...
				T offset,
				T h,
				tensorium::Tensor<T, 2>& g,
				tensorium::Tensor<T, 2>& g_inv,
