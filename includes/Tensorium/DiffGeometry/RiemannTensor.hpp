...
template <typename T> class RiemannTensor {
  public:
     ...
     * @brief Computes the 4D Riemann curvature tensor \f$ R^\rho_{\ \sigma\mu\nu} \f$
     *
     * This function numerically computes the full 4D Riemann tensor using finite difference
     * approximations (via Richardson extrapolation) on the Christoffel symbols computed from the
     * metric tensor.
     *
     ...
     * @param metric A callable metric object (e.g. Kerr, Schwarzschild) implementing operator()(X,
     * g)
    ...
    static Tensor4D compute(const VectorT &X, T h, const Metric<T> &metric) {
        ...
        Tensor2D         g({dim, dim});
        metric(X, g);
        Tensor2D g_inv = tensorium_RG::inv_mat_tensor_local(g);

        std::array<ChristoffelSym<T>, 4> gamma_ph{ChristoffelSym<T>(h), ChristoffelSym<T>(h),
        ...
        for (size_t mu = 0; mu < dim; ++mu) {
            ...
            auto inv_gh = tensorium_RG::inv_mat_tensor_local(gh);
            auto inv_gl = tensorium_RG::inv_mat_tensor_local(gl);
            auto inv_gh2 = tensorium_RG::inv_mat_tensor_local(gh2);
            auto inv_gl2 = tensorium_RG::inv_mat_tensor_local(gl2);
        ...
        for (size_t rho = 0; rho < dim; ++rho)
            for (size_t sigma = 0; sigma < dim; ++sigma)
                for (size_t mu = 0; mu < dim; ++mu)
                    for (size_t nu = 0; nu < dim; ++nu) {
                        T dGamma_mu = tensorium::richardson_derivative(
                            gamma_ph[mu](rho, nu, sigma, 0), gamma_mh[mu](rho, nu, sigma, 0),
                            gamma_ph2[mu](rho, nu, sigma, 0), gamma_mh2[mu](rho, nu, sigma, 0), h);

                        T dGamma_nu = tensorium::richardson_derivative(
                            gamma_ph[nu](rho, mu, sigma, 0), gamma_mh[nu](rho, mu, sigma, 0),
                            gamma_ph2[nu](rho, mu, sigma, 0), gamma_mh2[nu](rho, mu, sigma, 0), h);

                        T sum = dGamma_mu - dGamma_nu;

                        for (size_t lambda = 0; lambda < dim; ++lambda) {
                            sum += gamma_ph[mu](rho, mu, lambda, 0) *
                                   gamma_ph[nu](lambda, nu, sigma, 0);
                            sum -= gamma_ph[nu](rho, nu, lambda, 0) *
                                   gamma_ph[mu](lambda, mu, sigma, 0);
                        }
