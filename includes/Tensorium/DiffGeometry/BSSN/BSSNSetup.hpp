/**
 * @file BSSN.hpp
 * @brief Core implementation of the BSSN (Baumgarte–Shapiro–Shibata–Nakamura) formalism for general
 * relativity.
 *
 * This file defines the BSSN grid structure and the initialization logic required to populate BSSN
 * variables from a given metric. It computes the conformal metric, its derivatives, the conformal
 * Christoffel symbols, the extrinsic curvature tensor \( K_{ij} \), and the trace-free conformal
 * extrinsic curvature tensor \( \tilde{A}_{ij} \).
 */
...
struct alignas(32) BSSNGrid {
    ...
    std::vector<tensorium::Tensor<double, 2>>
        gamma_tilde; ///< Conformal metric \f$\tilde{\gamma}_{ij}\f$
    std::vector<tensorium::Tensor<double, 2>>
        gamma_tilde_inv; ///< Inverse \f$\tilde{\gamma}^{ij}\f$
    std::vector<tensorium::Tensor<double, 3>>
        dgamma_tilde; ///< Derivatives \f$\partial_k \tilde{\gamma}_{ij}\f$
    std::vector<tensorium::Tensor<double, 3>>
        christoffel_tilde; ///< Christoffel symbols \f$\tilde{\Gamma}^k_{ij}\f$
    std::vector<tensorium::Tensor<double, 2>> ExtrinsicTensor; ///< Extrinsic curvature \f$K_{ij}\f$
    std::vector<tensorium::Tensor<double, 2>>
        A_tildeTensor; ///< Trace-free extrinsic curvature \f$\tilde{A}_{ij}\f$
 ...
 *
 * This class initializes all core variables required for BSSN evolution using a given spacetime
 * metric. The metric must provide access to the lapse, shift, and spatial metric via
 * `metric.BSSN(X, α, β^i, γ_ij)`.
 *
