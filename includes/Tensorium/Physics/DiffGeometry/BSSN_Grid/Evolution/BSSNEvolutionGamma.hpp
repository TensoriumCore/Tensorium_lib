 ...
 */
template <typename T>
inline void compute_rhs_Gamma(const BSSNGridSoA<T> &G, Field3D<T> rhs[3], const Field3D<T> Z[3],
                              const Field3D<T> &Theta, const GaugeParameters<T> &gauge_params = {},
                              size_t padding = 4) {
    ...
    auto loop_ij = [&](size_t i, size_t j) {
        ...
        const T *p_Gamma[3] = {G.tildeGamma[0].ptr() + idx_start, G.tildeGamma[1].ptr() + idx_start,
        ...
        for (size_t k = k0; k < k1; ++k) {
            ...
            if (((i - i0) % gamma_validation_stride == 0) &&
                ...
                T A_up_ref[3][3];
                tensorium_RG::bssn::validate_A_raising(G, i, j, k, ginv_matrix, A_lo_matrix,
...
#pragma omp simd
            ...
            recover_z_over_chi_from_gamma_ptr(p_Gamma[0], p_Gamma[1], p_Gamma[2], p_ginv[0],
                                              p_ginv[1], p_ginv[2], p_ginv[3], p_ginv[4], p_ginv[5],
                                              sx, sy, inv_12dx, inv_12dy, inv_12dz, z_over_chi,
                                              gamma_metric);
            ...
            for (int comp = 0; comp < 3; ++comp) {
                const T adv = beta_vec[0] * Dx_upwind_ptr(p_Gamma[comp], sx, inv_2dx, beta_vec[0]) +
                ...
                const T theta_drive =
                    T(2) * alpha * grad_theta_up - T(2) * theta_val * grad_alpha_up;
