...
class TensoriumASTConsumer : public ASTConsumer {
  public:
    ...
    void HandleTranslationUnit(ASTContext &Context) override {
        ...
        Finder->addMatcher(varDecl(hasType(recordDecl(hasName("::std::vector"))),
                                   unless(hasType(TypeMatcher(
                                       hasDescendant(recordDecl(hasName("aligned_vector")))))))
                               .bind("unaligned_vector"),
                           Checker);

        Finder->addMatcher(
            varDecl(hasType(pointerType(pointee(builtinType())))).bind("raw_pointer"), Checker);

        Finder->addMatcher(varDecl(hasType(qualType(hasDeclaration(classTemplateSpecializationDecl(
                                       matchesName("::tensorium::(Vector|Matrix|Tensor)"))))))
                               .bind("tensorium_type"),
                           Checker);

        Finder->addMatcher(
            varDecl(unless(hasType(qualType(hasDeclaration(classTemplateSpecializationDecl(
                        matchesName("::tensorium::(Vector|Matrix|Tensor)")))))))
                .bind("non_tensorium"),
            Checker);
...
dispatch_simd([](auto simd){
    ...
    static void injectRestrict(Preprocessor &PP) {
        ...
        if (!code.empty())
            pushBuffer(PP, code, "morph_restrict");
    }
