...
tensorium::Vector<double> numpy_to_vector(py::array_t<double> arr) {
    ...
    return tensorium::Vector<double>(std::vector<double>((double*)buf.ptr, (double*)buf.ptr + buf.shape[0]));
}
...
PYBIND11_MODULE(tensorium, m) {
    ...
    tns.def("compute_christoffel", [](const Vector<double>& X,
                                         const tensorium::Tensor<double, 2>& g,
                                         const tensorium::Tensor<double, 2>& ginv,
                                         const std::string& metric_type,
                                         double M, double a) {
        auto metric = tensorium_RG::Metric<double>(metric_type, M, a);
        return tensorium::compute_christoffel(X, 1e-5, g, ginv, metric);
    });
    tns.def("compute_riemann_tensor", [](const Vector<double>& X,
                                            const std::string& metric_type,
                                            double M, double a) {
        auto metric = tensorium_RG::Metric<double>(metric_type, M, a);
        return tensorium::compute_riemann_tensor<double>(X, 1e-5, metric);
    });
	tns.def("contract_riemann_to_ricci", 
	...
	tns.def("print_ricci_tensor", &tensorium::print_ricci_tensor<double>, 
			py::arg("R"), "Print components of the Ricci tensor");


	tns.def("print_ricci_scalar",
			&tensorium::print_ricci_scalar<double>,
    ...
    tns.def("linear_comb", &tensorium::linear_combination_vec<float>, "Linear combination of vectors");
    tns.def("cross", &cross_vec<float>, "Cross product (only defined for 3D vectors)");

	tns.def("tensor_product", [](const tensorium::Tensor<double, 2>& A,
				const tensorium::Tensor<double, 2>& B) {
    ...
    tns.def("jacobi_solve", &jacobi_solve<float>, py::arg("A"), py::arg("b"), py::arg("tol") = 1e-6f, py::arg("max_iter") = 1000, "Jacobi iterative method");
	tns.def("row_echelon", &tensorium::row_echelon<float>,
